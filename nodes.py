...
def _get_ollama_client() -> Client:
    try:
        return Client(host=config.OLLAMA_HOST, headers=config.OLLAMA_AUTH_HEADER)
    except Exception as exc:
        logging.exception("Failed connection to Ollama")
        raise RuntimeError(
            "Failed to connect to Ollama. Check OLLAMA_HOST and (if needed) OLLAMA_BEARER_TOKEN."
        ) from exc
